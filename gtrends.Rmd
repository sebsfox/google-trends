...
p <- ggplot(dfgraph, aes(x = fingertips_scaled, y = google_scaled, label = label)) +
        geom_point() +
        labs(x = "2.12 - Percentage of adults classified as overweight or obese",
             y = "Google trends searches for diet and lose weight") + 
        geom_text(aes(x = min(dfgraph$fingertips_scaled) * 1.05, y = max(dfgraph$google_scaled) * 0.95,
                      label = paste("R^2:", round(summary(lm(google_scaled ~ fingertips_scaled,
